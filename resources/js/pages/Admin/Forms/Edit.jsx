...
export default function FormEdit({ form: initialForm }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    put(route('admin.forms.update', initialForm.id));
  };

  return (
    <AdminLayout title={`Redigera: ${initialForm.title}`}>
      <Head title={`Redigera: ${initialForm.title}`} />
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Redigera formulär</h1>
            <p className="text-muted-foreground mt-1">{initialForm.title}</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Form Settings - Same as Create.tsx */}
          <Card>
            <CardHeader>
          ...
          {/* Questions - Same structure as Create.tsx */}
          <Card>
            <CardHeader>
                      ...
                      {question.type === 'slider' && (
                        <div className="grid grid-cols-5 gap-4">
                          <div>
                            ...
                            <Input
                              type="number"
                              value={(question.config && question.config.min) || 0}
                              onChange={(e) =>
                          ...
                          <div>
                            ...
                            <Input
                              type="number"
                              value={(question.config && question.config.max) || 100}
                              onChange={(e) =>
                                updateQuestion(qIndex, 'config', {
                                  ...(question.config || {}),
                          ...
                          <div>
                            ...
                            <Input
                              type="number"
                              value={(question.config && question.config.default) || 0}
                              onChange={(e) =>
                                updateQuestion(qIndex, 'config', {
                                  ...(question.config || {}),
                          ...
                          <div>
                            ...
                            <Input
                              type="number"
                              value={(question.config && question.config.step) || 1}
                              onChange={(e) =>
                                updateQuestion(qIndex, 'config', {
                                  ...(question.config || {}),
                          ...
                          <div>
                            <Label>Enhet</Label>
                            <Input
                              value={(question.config && question.config.unit) || ''}
                              onChange={(e) =>
                                updateQuestion(qIndex, 'config', {
                                  ...(question.config || {}),
                                  unit: e.target.value,
                      ...
                      {['text', 'email', 'phone', 'textarea'].includes(question.type) && (
                        <div className="space-y-3">
                          <div>
                            ...
                            <Input
                              value={(question.config && question.config.placeholder) || ''}
                              onChange={(e) =>
                                updateQuestion(qIndex, 'config', {
                                  ...(question.config || {}),
                          ...
                          <div className="flex items-center space-x-2">
                            <Switch
                              checked={(question.config && question.config.validate) || false}
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={processing || data.questions.length === 0}>
              {processing ? 'Sparar...' : 'Uppdatera formulär'}
            </Button>
          </div>
        </form>
      </div>
